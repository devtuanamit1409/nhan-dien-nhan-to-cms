...
module.exports = {
  ...
  async beforeCreate(event) {
    ...
    if (data.birthDate) {
      // Lấy 4 kí tự cuối của birthDate (năm sinh)
      const yearOfBirth = parseInt(data.birthDate.slice(-4), 10);

      if (!isNaN(yearOfBirth)) {
        const currentYear = new Date().getFullYear();
        data.age = currentYear - yearOfBirth;
      } else {
        strapi.log.warn(
          "birthDate is not in the correct format to extract the year."
        );
      }
    }
  },
